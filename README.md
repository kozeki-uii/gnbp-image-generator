...
- 文生图、图片编辑、多张参考图和批量生成
- 支持 `Ctrl+V` 直接粘贴剪贴板图片，也可把本地图片拖入窗口
...
也可以直接按 `Ctrl+V` 粘贴截图/剪贴板图片，或者把 JPG、PNG、WebP 等图片
从资源管理器拖入客户端。桌面端默认请求超时为 1800 秒，设置中最高可调整为
3600 秒；如果请求总是在约 60 秒时显示 `Failed to fetch`，通常是中转站的
Nginx/CDN 提前断开，仍需调整中转服务超时或启用其支持的流式传输。
